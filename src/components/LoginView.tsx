...
export const LoginView: React.FC<LoginViewProps> = ({
  ...
  return (
    <div className="relative min-h-screen w-full bg-[#07090E] text-white flex flex-col items-center justify-center p-4 sm:p-6 overflow-x-hidden select-none">
      ...
      {loginMedia?.youtubeEnabled && youtubeId && youtubeId.trim() ? (
        <div className="fixed inset-0 z-0 overflow-hidden bg-black">
          <iframe
            src={`https://www.youtube.com/embed/${youtubeId.trim()}?autoplay=1&mute=1&controls=0&cc_load_policy=0&playsinline=1&loop=1&playlist=${youtubeId.trim()}&fs=0&disablekb=1&iv_load_policy=3`}
        ...
        <div 
          ...
          style={{
            backgroundImage: `radial-gradient(circle at 50% 30%, rgba(245, 197, 66, 0.08) 0%, transparent 60%), radial-gradient(circle at 20% 80%, rgba(30, 41, 59, 0.4) 0%, transparent 50%), url('${(loginMedia?.backgroundImageUrl && loginMedia.backgroundImageUrl.trim()) || 'https://images.unsplash.com/photo-1600585154340-be6161a56a0c?auto=format&fit=crop&w=1920&q=80'}')`
          }}
        />
      )}
